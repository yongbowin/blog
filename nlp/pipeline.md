...
**一般概念：**
 ...
 - 可分为两类：
    - （Boosting，串行）个体学习器间存在强依赖关系，必须采用串行生成的序列化方法
    - （Bagging和随即森林，并行）个体学习器间不存在强依赖关系，可同时生成的并行化方法
...
**Bagging与随机森林：**
 ...
 - Bagging：
    ...
    - Bagging主要**关注降低方差**，
 - 随机森林：
    - 是Bagging的一个扩展变体，**在以决策树为基学习器构建Bagging集成的基础上，进一步在决策树的训练过程中引入了随机属性选择**
